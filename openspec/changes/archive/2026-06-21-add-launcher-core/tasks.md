...
- [x] 10.6 人工验证 Option + Space、快捷键冲突与旧配置保留。
  - 用户于 2026-06-21 确认人工验收完成；真实 HID Option + Space 事件也验证了面板唤起与再次按键关闭。
- [x] 10.7 人工验证输入焦点、Up/Down/Return/Escape、鼠标操作和重复唤起。
  - 用户于 2026-06-21 确认人工验收完成；真实应用辅助功能树确认搜索框自动聚焦，方向键选择、Return 执行、Escape 关闭与鼠标点击均正常。
- [x] 10.8 人工验证应用、文件、命令结果以及 Spotlight 受限降级。
  - 用户于 2026-06-21 确认人工验收完成。
- [x] 10.9 人工验证六个命令进入正确页面，且不提前执行对应业务功能。
  - 2026-06-21 在真实应用中逐一执行打开剪切板、扫描磁盘、卸载应用、权限审计、查看微信占用和打开系统监控；同一主窗口进入对应占位页，未触发扫描、删除、审计、微信读取或高频监控。
- [x] 10.10 人工验证多显示器、Space、浅色/深色与 VoiceOver 基础体验。
  - 用户于 2026-06-21 确认人工验收完成。
- [x] 10.11 审阅任务清单，确保完成状态和验收证据准确。
- [x] 10.12 验收后将 launcher 规范合并到 `openspec/specs/launcher/spec.md` 并归档 change。
  - 2026-06-21：人工验收与全量测试完成，本次归档将规范合并到主规范目录。

## 11. 输入法组合态搜索与应用搜索性能优化

> 本节为后续补充任务，已于 2026-06-21 完成实现、自动化测试与人工验收。
...
- [x] 11.13 人工验证系统拼音输入法下无需确认候选或切换输入法即可用 `wechat` 搜索微信，并验证候选键盘操作不受影响。
  - 用户于 2026-06-21 确认人工验收完成。
- [x] 11.14 执行构建与全部测试，记录验收证据后更新任务状态。
  - 2026-06-21：执行完整 `xcodebuild test`，全部测试通过；Launcher 预热应用匹配 P95 与首批本地结果 P95 性能测试通过。

## 12. 人工验收缺陷修复

- [x] 12.1 修复主窗口无法观察 Launcher 命令导航目标的问题，确保面板执行命令后立即切换目的地。
  - `MainWindowNavigator` 改用 SwiftUI Observation；已有 SwiftUI 主窗口标识按受控前缀识别，只前置现有窗口且不创建重复窗口。
- [x] 12.2 增加真实状态传播边界测试，覆盖导航目标发布、消费和连续命令。
  - `LauncherCommandExecutorTests` 覆盖 Observation 变化通知、目标消费以及 SwiftUI 主窗口标识识别。
- [x] 12.3 重新执行六个命令的真实应用验收，确认仅导航且不触发对应业务操作。
  - 2026-06-21 真实应用逐项验收通过，六次执行始终复用 `omnipo.main-AppWindow-1`。
