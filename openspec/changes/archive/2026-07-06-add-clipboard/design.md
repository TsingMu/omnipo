...
- 按 `clippy` 交互方向提供独立剪切板悬浮面板
- 通过专用全局快捷键呼出/隐藏剪切板悬浮面板
- 状态栏最近记录菜单
...
`clippy` 的核心体验之一是用户按下专用快捷键后直接打开剪切板历史面板。Omnipo 应保留这个产品方向，但实现方式需要融入现有架构：

- 快捷键注册必须复用 Omnipo 现有 `ShortcutService` / `CarbonShortcutService` 能力，或在其上增加命名空间化的剪切板快捷键类型；不得迁移 `clippy` 的 `HotKeyManager` 或形成并行快捷键栈。
- 剪切板悬浮面板应作为新的展示入口复用 `ClipboardService`、Repository 与现有复制/粘贴动作；不得创建第二套剪切板监听、缓存或持久化逻辑。
- 快捷键触发时，若用户已完成本地存储确认并启用 Clipboard，则显示或切换悬浮面板，并默认聚焦搜索输入。
- 面板再次收到同一快捷键、按下 Escape、或失去有效交互焦点时应可隐藏；具体关闭策略应与 Launcher 面板保持一致。
- 若用户尚未完成首次本地存储确认，快捷键只能打开确认入口或剪切板页提示，不得启动监听或写入历史。
- 面板位置优先跟随当前活跃屏幕/窗口上下文；只有在真正提供独立面板后，才评估是否持久化 `panelPosition`。
...
不迁移 `clippy` 的 `HotKeyManager`。若后续要支持独立剪切板面板呼出，应复用 Omnipo 现有快捷键能力或在其之上扩展，例如增加 Clipboard panel action，而不是新增独立 Carbon event handler。

快捷键配置应进入 Omnipo 统一快捷键设置模型；`clippy` 的 `HotKeyConfig` 只作为语义参考，不作为迁移对象。
...
- 辅助功能权限引导
- 复制并粘贴
- 按 `clippy` 体验实现或评估快捷键调起剪切板悬浮面板：专用快捷键切换面板、默认聚焦搜索、Escape/重复快捷键关闭
- 若引入独立面板，再评估 `panelPosition` 和状态栏设置的整合方式
