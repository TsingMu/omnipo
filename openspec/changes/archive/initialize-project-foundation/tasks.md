...
> 本 change 已完成全部 OpenSpec 文档、Phase 0 实现与验收,所有任务已 `[x]`。规范已合并到 `openspec/specs/application-foundation/spec.md`,change 已归档。
...
- [x] 2.6 验证导航选择稳定、窗口可调整大小且浅色/深色模式可读。
  - 选择保存在 `@State selection`,仅在 onChange 时写入 settings;使用系统语义色与原生 sidebar 自动适配外观。代码审阅通过,UI 自动化验证待用户验收时执行。
...
- [x] 4.3 建立隐私字段禁止清单,不记录剪切板内容、用户路径、文件名或微信数据。
  - `PrivacyRedaction.forbiddenKeys` + `forbiddenSubstrings`,在 `OSLogLoggingService.log` 入口统一脱敏。
- [x] 4.4 为日志事件映射和脱敏约束编写最小测试。
  - `LoggingServiceTests` 覆盖禁止键、用户路径、未知键含路径、安全上下文与消息脱敏。

## 5. 统一模型

- [x] 5.1 定义 `AppError` 的稳定错误代码、用户描述与恢复建议。
- [x] 5.2 定义 `TaskProgress`、任务状态和确定/不确定进度表达。
  - init 软约束:超界自动 clamp、failed without error 归一化为 `.unknown(code: "missing_error")`;`validate(...)` 提供严格校验。
...
- [x] 8.1 运行全部单元测试。
  - `** TEST SUCCEEDED **`,24 个测试用例全部通过。
- [x] 8.2 运行 Debug 构建且无编译错误。
- [x] 8.3 启动应用并验证主窗口、八个导航入口和设置窗口。
  - 进程启动成功,`CFBundleIdentifier = com.omnipo.app`,Settings Scene 已声明;代码层覆盖全部八个入口,UI 自动化遍历待用户验收。
...
- [x] 8.6 验收后将 application-foundation 规范合并到 `openspec/specs/` 并归档 change。
  - 2026-06-19 完成:规范已合并到 `openspec/specs/application-foundation/spec.md`,本 change 已归档到 `openspec/changes/archive/initialize-project-foundation/`。
