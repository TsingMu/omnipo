...
- [ ] 3.1 实现 `ClipboardMonitor`，基于 `NSPasteboard.general.changeCount` 监听变化。
- [ ] 3.2 支持文本、RTF、HTML、图片和文件路径的读取与分类。
- [ ] 3.3 实现来源应用识别，并确保失败时不影响主流程。
- [ ] 3.4 实现记录写回系统剪切板能力。
- [ ] 3.5 实现自动粘贴，并在缺少辅助功能权限时降级为仅复制。
- [ ] 3.6 为监听、格式识别、写回和权限降级编写测试。
- [ ] 3.7 执行构建与测试并更新任务状态。

## 4. 首次使用提示与启用流程

- [ ] 4.1 在 Clipboard 页面增加首次使用提示，明确说明“仅本地存储”和敏感内容风险。
- [ ] 4.2 在用户确认前禁止启动监听和持久化。
- [ ] 4.3 用户确认后持久化本地确认状态，并启动 Clipboard 能力。
- [ ] 4.4 允许用户后续关闭 Clipboard 记录能力，并保留再次开启路径。
- [ ] 4.5 为首次提示、确认、关闭和再次启用流程编写测试。
- [ ] 4.6 执行构建与测试并更新任务状态。

## 5. 主窗口 Clipboard 页面

- [ ] 5.1 用真实 Clipboard 页面替换现有占位页。
- [ ] 5.2 实现搜索框、类型过滤和空状态。
- [ ] 5.3 实现记录列表、选中态、收藏和删除操作。
- [ ] 5.4 实现“复制到剪切板”和“复制并粘贴”交互。
- [ ] 5.5 为浅色/深色、键盘可达性和 VoiceOver 基础体验补充验证。
- [ ] 5.6 执行构建与测试并更新任务状态。
