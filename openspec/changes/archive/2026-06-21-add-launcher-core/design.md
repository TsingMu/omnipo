...
性能基准使用固定生成的 1,000 条应用记录，先完成索引预热和一次搜索热身，再采集至少 30 次 `ContinuousClock` 样本并按升序第 `ceil(n × 0.95)` 个样本计算 P95。应用匹配基准直接测量 `ApplicationSearchProvider`；首批可见结果基准从 `LauncherStore.updateQuery` 开始，直到 Store 收到非空本地批次，文件提供者保留 debounce 以验证 Spotlight 不进入该预算。基准不扫描真实应用目录、不查询真实 Spotlight，避免机器外部状态污染回归判断。
...
主窗口导航状态必须使用 SwiftUI Observation 可直接追踪的单一状态源。`RootView` 观察的导航对象不得仅依赖嵌套 `ObservableObject` 发布，否则环境容器不会把内部 `@Published` 变化传播为视图更新。命令执行器必须发布一次激活请求和一次导航目标，主窗口消费目标后再清空；已有主窗口只前置，不得发出新建窗口请求或创建重复窗口。面板关闭不得成为导航刷新的隐式条件。
