...
把系统监控从“单页多卡片”调整为“纵览 + 资源专项标签页”的结构，同时在资源专项页补上对应维度的当前 APP 使用排行。设计继续坚持三条边界：
...
纵览页展示 CPU、内存、能耗、磁盘、网络的紧凑状态卡或摘要行，只表达整机总体使用情况，不展示应用排行。

### 资源专项页

CPU、内存、能耗、磁盘、网络页分别展示现有卡片中的详细内容。专项页可复用当前卡片组件，但每页只聚焦一个维度，避免用户在长列表中查找。具备可靠应用级采样的专项页在详情下方展示对应维度的 APP 排行，例如 CPU 页按 CPU 占有率降序，内存页按内存占用降序；无法可靠归因的维度必须显示降级原因。
...
`usageAmount` 是模型层默认排序的当前采样值，不是历史使用时长。MVP 可采用以下稳定规则：

1. 优先以 CPU 当前占用作为主要使用量。
2. CPU 相同或接近时以内存占用作为次级排序。
3. 仍相同时按应用名称升序，保证排序稳定。

UI 专项页必须按当前资源维度排序：CPU 页按 CPU 占有率降序，内存页按内存占用降序。后续 change 可引入加权综合分，但必须重新定义权重与可解释性。首版 UI 文案应避免把 `usageAmount` 表述成“使用时长”。
...
AppUsageRecord {
    ...
    cpuPercent: Double? // Activity Monitor scale: 1.0 is 100% of one CPU core and values may exceed 1.0.
    memoryBytes: Int64?
...
推荐采用第二种，原因是 APP 使用情况是资源专项页的增强模块，不应污染五维度整机模型。`SystemMonitorStore` 负责把同一采样周期内的整机快照与 APP 使用快照暴露给 UI。
...
- 位于对应资源专项页详情下方，不位于纵览页。
- CPU 页按 CPU 占有率降序；内存页按内存占用降序；网络、能耗、磁盘等无法可靠应用级归因时显示降级原因。
...
当前资源专项页的 APP 使用区块显示“APP 使用情况暂不可用”及原因；CPU、内存等整机指标继续展示。
...
- UI 状态测试：纵览页只显示整机总体摘要；CPU/内存专项页按对应维度展示 APP 排行；不可用和空状态不伪造记录；专项页只展示对应维度。
