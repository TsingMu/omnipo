...
- [x] 3.2 实现 `WeChatStorageScanner`，只读取文件系统元数据，不打开或解析文件内容；符号链接先解析真实路径并按真实路径去重，拒绝跟随指向候选 root 并集之外的链接，以 `externalLinkSkipped` 按 root 聚合，不误报权限不足。
...
- [x] 5.1 用真实 WeChat Manager 页面替换占位视图。
- [x] 5.2 实现刷新、扫描中、成功、空状态、部分不可用和失败状态；真实权限受限时提供系统设置引导，并支持用户通过目录选择器授权额外扫描根。
- [x] 5.3 展示总可见占用、分类占用、top storage groups 和不可用根说明。
- [x] 5.4 展示隐私边界：只统计文件元数据，不读取聊天内容或联系人。
- [x] 5.5 对 raw path 做弱化展示，避免把可能含账号或联系人信息的路径作为主文案。
- [x] 5.6 验证浅色/深色、键盘可达性和 VoiceOver 基础体验。

## 6. 日志、隐私与验收

- [x] 6.1 审计 WeChat Storage 日志，确认不包含用户路径、文件名、账号、联系人、消息或数据库内容。
- [ ] 6.2 人工验证无 WeChat 数据时显示“未发现”而不是错误。
- [x] 6.3 人工验证存在可读 fixture 或真实授权根时显示分类占用。
- [x] 6.4 人工验证不可读根显示明确原因，不误报为 0 B 或无数据。
...
- [x] 7.1 按图片、视频、文件、音频等类型汇总空间占用，并按匿名会话维度展示占用排行。
- [x] 7.2 提供大文件列表、类型筛选、大小门槛和排序，默认不暴露真实文件名。
- [x] 7.3 增加敏感名称显式授权；仅在授权后扫描并展示真实文件名，撤回授权后重新匿名扫描。
- [x] 7.4 支持按真实文件名搜索，并确保撤回授权后遗留搜索词不会过滤匿名结果。
- [x] 7.5 支持为匿名会话设置仅驻留内存的本地名称；撤回授权时立即清除，不持久化、不记录日志。
- [x] 7.6 检测到微信 4.x 加密会话数据库时不尝试提取密钥或解密，在界面明确说明真实聊天名称需由用户本地设置。
- [x] 7.7 为扫描选项、敏感文件名、授权设置、服务透传和会话名称生命周期补充单元测试。
- [x] 7.8 在真实应用中验证授权提示、文件名显示与搜索、会话名称编辑、撤回授权和匿名回退。
